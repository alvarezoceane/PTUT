...
def open_folders():  
    ...
    #data = Path("/home/alvarez/Desktop/PTUT/Minipatches_light_20260116") # Path is used to set our current working directory
...
def file_existence(folder:Path):
    ...
    has_traj_csv = int(traj_csv.is_file())
    has_traj_mat = int(traj_mat.is_file())
    has_foodpatches_reviewed_mat = int(foodpatches_reviewed_mat.is_file())
        
        
            
    
    
    return (traj_csv,traj_mat,foodpatches_reviewed_mat, has_traj_csv,has_traj_mat,has_foodpatches_reviewed_mat)
...
def threshold(folder: Path):
    traj_mat = folder / "traj.mat"
    if traj_mat.is_file() and traj_mat.stat().st_size <= 200:
        traj_mat_empty = 0
    else:
        traj_mat_empty = 1
...
def create_csv(file_path,file_name):
    ...
    csv_columns = [
        "Folder Path",
        "traj.csv exists?",
        "foodpatches_reviewed.mat exists?",
        "traj.mat exists?",
        "traj.mat empty?",
        "Diagnosis of video",
    ...
    # data = Path("/home/alvarez/Desktop/PTUT/Minipatches_light_20260116")
    data = Path("Documents/PTUT/Minipatches_light_20260116")
    ...
    for folder in open_folders():
        ...
        traj_csv,traj_mat,foodpatches_reviewed_mat,has_traj_csv,has_traj_mat,has_foodpatches_reviewed_mat= file_existence(folder)
        
        #print(has_traj_mat, has_foodpatches_reviewed_mat, has_traj_csv)

        traj_mat_empty = int(threshold(folder))
        
        print(has_traj_mat, has_foodpatches_reviewed_mat, has_traj_csv,traj_mat_empty)
        ...
        rows.append([  str(folder),
            has_traj_csv,
            has_foodpatches_reviewed_mat, has_traj_mat, traj_mat_empty, diagnosis,f0,f1,f2,prop_multiple,category ])
...
def histogram(file, seuil):
	# Lire le fichier CSV
	df = pd.read_csv(file)
	# Vérifier si la colonne existe dans le DataFrame
	if "proportion of frames with multiple worms" in df.columns:
		# Tracer l'histogramme
		df["proportion of frames with multiple worms"].hist(bins=20)
...
def speed_one_folder(folder: Path):
    ...
    # colonnes frame/frames (selon tes fichiers)
    frame_col = "frame" if "frame" in df.columns else ("frames" if "frames" in df.columns else None)
    ...
    # IMPORTANT: trier par frame pour être sûr de l’ordre
    ...
    return frames_mid, speed

def plot_all_speeds(root_folder, n):
    folders = open_folders()
    if len(folders) == 0:
        print("Aucun dossier trouvé dans:", root_folder)
        return

    chosen = random.sample(folders, k=min(n, len(folders)))
    print("Dossiers tirés au hasard:", chosen)

    plt.figure()

    plotted = 0
    for folder in chosen:
        frames_mid, speed = speed_one_folder(folder)
        if frames_mid is None:
            print("Skip (données manquantes):", folder)
            continue

        plt.plot(frames_mid, speed, alpha=0.6)
        plotted += 1

    plt.xlabel("Frames")
    plt.ylabel("Speed")
    plt.ylim(0, 1000)
    plt.title(f"Overlay of {plotted} trajectories")
    plt.grid()
    plt.show()
...
if __name__ == "__main__": # To prevent the function from running automatically when the file is imported somewhere else
    ...
    create_csv("Documents/PTUT","results_29.csv")
    #histogram("Documents/PTUT/results.csv" 0.05)
	#calculs("/home/alvarez/Desktop/PTUT/Minipatches_light_20260116")
    #calculs( "Documents/PTUT/Minipatches_light_20260116")
    #plot_all_speeds("Documents/PTUT/Minipatches_light_20260116", 10)
    #file_existence("Documents/PTUT/Minipatches_light_20260116")
